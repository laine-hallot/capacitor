...
const map_workspaces_1 = tslib_1.__importDefault(require("@npmcli/map-workspaces"));
...
async function getPlugins(config, platform) {
    var _a, _b, _c;
    if (((_a = config.app.workspaces) === null || _a === void 0 ? void 0 : _a.type) === 'npm') {
        const { workspaceDirs, workspaceRoot } = config.app.workspaces;
        const workspacePackages = await (0, map_workspaces_1.default)({
            cwd: workspaceRoot,
            pkg: { workspaces: workspaceDirs },
        });
        const resolvedWorkspacePackages = await Promise.all(Array.from(workspacePackages.entries()).map(async ([name, path]) => {
            return parsePluginMeta((0, path_1.join)(path, 'package.json'), name);
        }));
        const possiblePackageJsonPlugins = (_b = getIncludedPluginPackages(config, platform)) !== null && _b !== void 0 ? _b : getDependencies(config);
        const resolvedPackageJsonPlugins = await Promise.all(possiblePackageJsonPlugins.map(async (p) => resolvePlugin(config, p)));
        const plugins = [...resolvedPackageJsonPlugins, ...resolvedWorkspacePackages].filter((p) => p !== null);
        return plugins;
    }
    const possiblePlugins = (_c = getIncludedPluginPackages(config, platform)) !== null && _c !== void 0 ? _c : getDependencies(config);
...
const parsePluginMeta = async (packagePath, name) => {
    try {
